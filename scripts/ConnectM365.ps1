param(                         
    ...
    [string]$tenant = "tenant" ## if -tenant used then the tenant name will be used to search for sites
)
...
if ($tenant -eq "tenant") {
    ## if -tenant used then the tenant name will be used to search for sites
    ...
    foreach ($domain in $domains) {
        ...
        if ($domain.id.contains('onmicrosoft')) {
            ...
            $tenant = $onname[0]                    ## the first string in this array is the name of the tenant
        }                                               ## end of find the on.microsoft.com domain
    }


    write-host -foregroundcolor $processmessagecolor "Tenant name =", $tenant
}
else {
    write-host -foregroundcolor $processmessagecolor "Tenant name =", $tenant
}
...
(Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/sites?search=mongadev" -Method GET).value

write-host -foregroundcolor $processmessagecolor "Get all SharePoint Online sites"
$sites = (Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/sites?search=$($tenant)" -Method GET).value
$root = (Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/sites/root" -Method GET)
##$sites = get-mgsite -search $tenant
$siteSummary = @()
